...
SLM의 answer-only 재생성을 이용해 첫 출력과의 숫자 일치를 확인한다. 범용 임베딩은
의미적 유사성을 효과적으로 표현하지만~\cite{xiao2023bge}, 같은 주제의 질문도 SLM이
해결할 수 있는 난이도는 다를 수 있다. 초기 GSM8K 실험에서 frozen BGE 기반 logistic
router의 AUC는 0.589였고, 출력 confidence 기반 router는 0.722를 기록하였다.

\section{제안 방법}

그림~\ref{fig:architecture}는 제안 라우팅 구조를 보여준다. Lower SLM의 첫 출력으로
routing score를 계산하고, 높은 confidence의 요청은 즉시 채택한다. 경계 사례는 숫자
한 줄만 생성하는 독립 검증을 수행하며, 첫 답과 검증 답이 다르면 Upper LLM으로
전달한다.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig1_architecture.pdf}
  \caption{출력 confidence와 answer-only 검증을 결합한 제안 라우팅 구조.}
...
정책 선택 split에서 고정한 두 임계값은 $\tau_l=0.12$, $\tau_h=0.80$이다.
...
기준 비용은 $C_L=1$, $C_U=4.667$이며, 동일 GPU에서 측정한 answer-only/full-pass
latency 비율에 따라 $C_V=0.0591$을 사용한다. 추가로 $C_V=0.35$인 보수적 비용
민감도도 확인한다.
...
분리했으며, 공식 test 300문항은 최종 진단에만 사용하였다. 모든 split은 고정 seed
42와 문항 ID로 관리하였다.
...
greedy decoding으로 평가하였다. 첫 답은 최대 256토큰, answer-only 검증은 최대
64토큰으로 제한하였다.

평가 지표는 최종 accuracy, Upper-call rate, unsafe routing, Always-Upper 대비
normalized cost이다. latency는 워밍업 뒤 동일 128문항을 batch size 16으로 조건별
2회 ABBA 순서로 측정하였다. 통계 분석은 동일 문항에 대한 10,000회 paired bootstrap
신뢰구간과 exact McNemar 검정을 사용하였다. unsafe 위험에는 독립 250문항의 단측
...
\begin{figure}[t]
  ...
  \caption{동일 RTX 3090에서 full second pass와 answer-only verifier의 문항당
  지연시간 및 생성 토큰 수 비교.}
  \label{fig:latency}
\end{figure}
...
보인다. 본 연구는 selection split에서 품질과 unsafe 제약을 만족한 0.12/0.80 정책을
고정하였다.

\begin{table}[t]
\centering
\caption{Answer-only와 C3의 paired 비교(단위: percentage point)}
\label{tab:paired}
\small
...
평가하였다. 둘째, 공식 test 300문항은 정확도 비열등성을 검정하기에 충분하지 않다.
셋째, latency 측정은 단일 RTX 3090과 조건별 2회 반복에 한정되며 API 가격이나 에너지
소비와 동일하지 않다. 넷째, answer-only 정책은 5\% 위험 인증을 근소하게 통과하지
못했다.
...
향후에는 더 큰 독립 표본과 다양한 모델 조합에서 비열등성, 위험 통제 및 serving
