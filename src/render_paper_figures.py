...
def architecture_figure(output_dir: Path):
    ...
    ax.axis("off")
    nodes = [
        (0.2, 1.55, 1.45, 0.9, "Query", "input", "#F7F7F7"),
        (2.0, 1.55, 1.65, 0.9, "Lower SLM", "first answer", "#F2F2F2"),
        (4.0, 1.55, 1.7, 0.9, "Confidence", "output-aware", "#EDEDED"),
        (6.1, 1.55, 1.8, 0.9, "Answer-only", "verification", "#E8E8E8"),
        (8.35, 2.45, 1.6, 0.9, "Accept", "answers agree", "#F2F2F2"),
        (8.35, 0.65, 1.6, 0.9, "Upper LLM", "escalation", "#DDDDDD"),
        (10.35, 1.55, 1.45, 0.9, "Response", "final output", "#F7F7F7"),
    ]
    ...
    for start, end in arrows:
        ax.annotate("", xy=end, xytext=start, arrowprops={"arrowstyle": "->", "color": MID, "lw": 1.4})
    ax.text(8.0, 2.72, "agree", fontsize=7.5, color=DARK, ha="right")
    ax.text(8.0, 1.12, "disagree / low confidence", fontsize=7.5, color=MUTED, ha="right")
...
def latency_figure(data, output_dir: Path):
    ...
    for ax, values, ylabel, reduction in (
        ...
        ax.text(0.98, 0.92, reduction, transform=ax.transAxes, ha="right", color=BLACK, weight="bold")
...
def pareto_figure(data, output_dir: Path):
    ...
    ax.plot(costs, accuracies, "-o", color=DARK, markerfacecolor="white", linewidth=2, markersize=5, label="Selection Pareto")
    frozen = points[-2]
    ax.scatter([frozen["cost"]], [100 * frozen["accuracy"]], s=95, color=BLACK, edgecolor="white", linewidth=1.5, zorder=5, label="Frozen policy")
    ax.annotate("0.12 / 0.80", (frozen["cost"], 100 * frozen["accuracy"]), xytext=(-62, 14), textcoords="offset points", color=BLACK, weight="bold", arrowprops={"arrowstyle": "-", "color": BLACK})
    ax.axvline(1.0, color=MID, linestyle="--", linewidth=1, label="Always Upper cost")
...
def comparison_figure(data, output_dir: Path):
    ...
    for index, method in enumerate(("c3", "answer_only")):
        ...
        color = MID if method == "c3" else BLACK
    ...
    axes[1].axhline(1.0, color=MID, linestyle="--", linewidth=1)
